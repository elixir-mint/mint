defmodule Mint.HTTP2Test do
  ...
  describe "settings" do
    ...
    test "server can update the initial window size and affect open streams",
      ...
      assert_recv_frames(test_runner_ref, [settings(flags: flags)])
  ...
  describe "connection modes" do
    ...
    test "starting a connection with :passive mode and using recv/3", %{conn: conn, test_runner_ref: test_runner_ref} do
      {conn, ref} = open_request(conn)
      ...
      :ok = TestServer2.send_data(Process.get(@pdict_key), data)
    ...
    test "changing the mode of a connection with set_mode/2", %{conn: conn, test_runner_ref: test_runner_ref} do
  ...
  defp start_connection(context) do
    ...
    # SETTINGS here.
    conn =
      if connect_options[:mode] == :passive do
    ...
    [conn: conn, test_runner_ref: test_runner_ref]
  end
  ...
  defp recv_frames(ref, n) do
    receive do
      {^ref, {:frames_available, c}} when c >= n ->
        ...
        TestServer2.recv_next_frames(server, n)
      ...
      msg ->
        IO.puts(
          :stderr,
          "[WARNING] #{inspect(self())} ref #{inspect(ref)} recv_frames unhandled msg #{inspect(msg)}"
        )
  ...
  defp start_http2_server(args, test_runner_ref) do
    %{id: http2_server_id} = http2_server_spec = TestServer2.child_spec(args)
