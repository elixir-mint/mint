defmodule Mint.HTTP2.TestServer2 do
  use GenServer
  ...
  def handle_continue(
    ...
    {:ok, server_socket, frames} = accept_and_handshakes(listen_socket, server_settings, frames)

    :ok = send_frames_event(test_runner, frames)
  ...
  def handle_info(
    ...
    frames = decode_frames(data <> data_in, frames)
    :ok = send_frames_event(test_runner, frames)
  ...
  defp accept_and_handshakes(listen_socket, server_settings, frames) do
    ...
    {:ok, frames} = send_settings(server_socket, server_settings, frames)
  ...
  defp send_settings(server_socket, server_settings, frames) do
    import Mint.HTTP2.Frame, only: [settings: 1]

    ack_flags = Frame.set_flags(:settings, [:ack])

    # We reply with our SETTINGS.
    :ok = :ssl.send(server_socket, Frame.encode(settings(params: server_settings)))

    # We get the SETTINGS ack.
    frames = recv_and_decode(server_socket, "", frames)
    {{:value, settings_frame}, frames} = :queue.out(frames)
    assert settings(flags: ^ack_flags, params: []) = settings_frame

    # We send the SETTINGS ack back.
    :ok = :ssl.send(server_socket, Frame.encode(settings(flags: ack_flags, params: [])))
  ...
  defp recv_and_decode(server_socket, data, frames) do
    ...
    case decode_frames(data_in, frames) do
      :more ->
        recv_and_decode(server_socket, data <> data_in, frames)
  ...
  defp send_frames_event(_test_runner, {:more, _frames, _data}) do
    :ok
  end

  defp send_frames_event(test_runner, frames) do
  ...
  defp send_msg({pid, ref}, msg0) when is_pid(pid) and is_reference(ref) do
    ...
    # IO.puts(:stderr, "@@@@@@@@ TestServer2 #{inspect(self())} send_msg pid #{inspect(pid)} msg1 #{inspect(msg1)}")
    send(pid, msg1)
    :ok
  end
end
