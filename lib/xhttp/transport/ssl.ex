defmodule XHTTP.Transport.SSL do
  ...
  def default_ciphers() do
    :ssl.cipher_suites()
    |> Enum.reject(&(suite_to_str(&1) in @blacklisted_ciphers))
  end

  defp suite_to_str({kex, cipher, mac, prf}) when mac in [:aead, :null] do
    String.upcase("TLS_#{kex}_WITH_#{cipher}_#{prf}")
  end

  defp suite_to_str({kex, cipher, mac, _prf}) do
    String.upcase("TLS_#{kex}_WITH_#{cipher}_#{mac}")
  end

  defp suite_to_str({kex, cipher, mac}) do
    String.upcase("TLS_#{kex}_WITH_#{cipher}_#{mac}")
  end
end
